...
#HP_Summary_ASHP = HP_Summary["RHPP Name"][HP_Summary["Heat pump Type"] == "ASHP"]
...
#def getlist():
#    lst = []
#    for f in range(0, len(HP_Summary_ASHP)):
#        try:
#            lst.append(HP_Summary_ASHP.iloc[f][-4:])
#            print(f)
#        except:
#            print()
#    return lst
...
#ashpsAll=getlist()
#
#ASHPIDs=list(set(ashpsAll) & set(IDs))
#
## This function creates a DataFrame with all Domestic ASHP
##def createDataFrame(ASHPIDs):
...
#
#     HP_Out = HP_RawFile["HPTotDem"].resample('10T').pad() / 1000 *30   # Convert Wh/10mins into kW
#     HP_DataFrame = pd.concat([HP_DataFrame, HP_Out], axis=1, join="outer", sort=False
#)
#     
##
#HP_DataFrame.columns = ASHPIDs
...
#
#pickle_out = open("../../Data/HP_DataFrame_10mins.pickle", "wb")
...
### HP_reduced = HP_reduced[HP_reduced]
### HP_DataFrame = HP_DataFrame[HP_reduced.index].loc[dt]
#######====aa=HP_DataFrame.sum().sort_values().index[-3:]
#######====HP_DataFrame=HP_DataFrame.drop(aa,axis=1)
#
...
for z in HP_DailyDataFrame:
    allmeans.append((HP_DailyDataFrame[z].sum(axis=1)/6).mean())
...
plt.ylabel("10-minutely sampled demand (kW)", fontsize=15)
plt.xlim([0, tsamp])
plt.ylim([0, 4])
...
histo30, binz30 = np.histogram(allmeans, bins=range(0, int(max(allmeans)+20), 1))
...
for t in ax.yaxis.get_majorticklabels():
    t.set_fontsize(11)
plt.plot([np.mean(allmeans),np.mean(allmeans)],[0,10], color='black', linewidth=1,label='Mean')
plt.legend()
plt.grid(linewidth=0.2)
plt.tight_layout()

print(np.mean(allmeans))
#
