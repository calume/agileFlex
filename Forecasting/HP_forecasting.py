...
def seasonal():
    ...
    for i in smkeys:
        ...
        for z in range(0, len(keepdays[i])):
            totalHP[i]["HPNorm_kW_hh"].append(
                HP_DataFrame[i][keep_HP[i][z]]
                .iloc[keepdays[i][z] * 48 : (keepdays[i][z] + 1) * 48]
                .sum(axis=1)
                / len(keep_HP[i][z])
            )
            totalHP[i]["HPNorm_Date"].append(
                HP_DataFrame[i].iloc[keepdays[i][z] * 48].name
            )
    return totalHP
    ...
    for d in range(1, len(totalHP[k]["HPNorm_kW_hh"][:-1])):
        ...
        if todayBin != tomorrowBin:
            ...
            for i in range(0, len(MatchDays[d]["Date"])):
                ...
                matchday = HP_Normdate_Series[
                    HP_Normdate_Series == MatchDays[d]["Date"][i]
                ]
