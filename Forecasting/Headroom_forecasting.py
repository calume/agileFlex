...
def hdrm_forecast(Network,Case):
    ...
    pick_in = open("../Data/"+str(Network+Case)+"Winter15_10mins_Advance.pickle", "rb")
    WinterInputs = pickle.load(pick_in)
    
    pick_in = open("../Data/"+str(Network+Case)+"Winter14_10mins_Advance.pickle", "rb")
    ...
    #    "negative": True
    #}
    datess=[wkd_dates,wknd_dates]
    tempss=[wkd_temps,wknd_temps]
    names=['wkd','wknd']
    settings=Settings["winter Hdrm"]
    for g in range(0,2):
        settings['dates']=datess[g]
        settings['temps']=tempss[g]
        tsamp=144
        dailyrange = {}
        DailyDelta = {}
        DailyByBin = {}
    ...
    #        str(settings["Title"])
    #        + str(settings["Q"][:-1])
    #        + " (kWs) vs Settlement Period - "+str(names[g])
    #    )
        if settings["negative"]==True:
            tempLabels=tempLabels[::-1]
            cols = cols[::-1]
        for c in settings['data'].columns:
            dailyrange = range(0, len(settings['dates']), tsamp)
        
            DailyDelta[c] = pd.DataFrame(index=settings['dates'][dailyrange], columns=range(0, tsamp))
    ...
    #        plt.xticks(range(0,tsamp,int(tsamp/6)),times)
            for z in tempLabels:
                datesBinned[z] = TempBins[0][TempBins[0] == z].index
                DailyByBin[c][z] = pd.DataFrame(index=datesBinned[z], columns=range(0, tsamp))
                DailyByBin[c]["P95-" + str(z)] = pd.Series(index=range(0, tsamp))
                DailyByBin[c]["P5-" + str(z)] = pd.Series(index=range(0, tsamp))
                DailyByBin[c]["Median-" + str(z)] = pd.Series(index=range(0, tsamp))
    ...
    #                plt.fill_between(range(0,tsamp),0,y, facecolor=cols[n])
...
#                vals = DailyDelta[c].mean(axis=1).values
...
#        plt.tight_layout()
        ...
        pickle_out = open("../Data/"+str(Network+Case)+"_WinterHdrm_"+str(names[g])+".pickle", "wb")
        pickle.dump(DailyByBin, pickle_out)
        pickle_out.close()
    
    return wkd_temps,wknd_temps