...
SummerInputs['pv_delta']=SummerInputs['pv_delta'][:-1]*-1

summer_dates=SummerInputs['pv_delta'].index
winter_dates=WinterInputs['demand_delta'].index
...
plt.figure
plt.hist(all_temp, bins=10)

def advance_forecast(dates,data,temps,plot_title,units):
    dailyrange={}
    DailyDelta={}
    DailyByBin={}
    cols=['b','c','g','r']
    ...
    plt.figure(str(plot_title)+' P99 requirement (kWs) vs Settlement Period')
    ...
    for c in data.columns:
        ...
        for z in tempLabels:
            ...
            lbl=str(round(TempBins[1][z-1],1))+' - '+str(round(TempBins[1][z],1))+str(units)
    ...
    plt.tight_layout()
    plt.legend()
    plt.figure(str(plot_title)+str(units)+' vs max daily adjust (kW)')   
    u=0
    for c in data.columns:
        print(u)
        u=u+1
...
#DailyByBin_All=advance_forecast(summer_dates, SummerInputs['pv_delta'], all_rad,' Summer Demand Turn-up: ',' kW-hr/m^2/day')