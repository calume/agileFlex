...
for item in Site_Names:
    ...
    PVAll['P_GEN_Norm'][PVAll['Substation']==item]=PVAll['P_GEN'][PVAll['Substation']==item]/PVCapacities[item]
    PVOutput_BySiteName[item]['P_kW']=PVAll['P_GEN'][PVAll['Substation']==item]
    PVOutput_BySiteName[item]['P_Norm']=PVAll['P_GEN_Norm'][PVAll['Substation']==item]
    PVOutput_BySiteName[item]=PVOutput_BySiteName[item].resample('30Min')
    PVOutput_BySiteName[item]=PVOutput_BySiteName[item].interpolate(method='linear', limit=1)
...
pickle_out = open("Data/Pickle/PV_BySiteName.pickle","wb")
...
for z in range(0,len(Distkeys)):
    ...
    for i in range(0,1440,30):
        a=PVS[Distkeys[z]][PVS[Distkeys[z]].index.hour*60 + PVS[Distkeys[z]].index.minute==i].values
...
for item in PVOutput_BySiteName:
    plt.figure(item)
    plt.scatter(PVOutput_BySiteName[item]['P_kW'].index,PVOutput_BySiteName[item]['P_kW'],s=0.5, c=colors[n])
...
for item in qrts:
    ...
    plt.figure(item)
    for z in qrts[item]:
        plt.plot(qrts[item][z], color=colors[n], linewidth=1.8, label=z, linestyle=style[n])
        n=n+1
    plt.legend()
