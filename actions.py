        ...
        while Vmax[:, p] > 1.1 or Vmin[:, p] < 0.94 or CurMax[:, p] > 0.5:
            ...
            while Vmin[:, p] < 0.94:
                ...
                if len(UpInd) == 0:
                    ...
                    LoadBusByPhase = summary(
                        LoadsIn, Violations, CurMax, Vmax, Vmin, P, Gen, PO, LB, UB
                    )
                if len(DownInd) == 0:
                ...
                for z in UpInd:
                    if (
                        LoadBusByPhase[p]["VMin-Dem-UP"].loc[z] > 0
                        and LoadBusByPhase[p]["VMin-Dem-UP"][z]
                        > LoadBusByPhase[p]["VMin-Dem-DOWN"][z]
                    ):
                        u[z] = (
                            -(0.94 - Vmin[:, p])
                            / LoadBusByPhase[p]["VMin-Dem-UP"][z]
                            / (len(UpInd) * grad)
                        )
                ...
                for z in DownInd:
                    if (
                        LoadBusByPhase[p]["VMin-Dem-DOWN"][z] > 0
                        and LoadBusByPhase[p]["VMin-Dem-DOWN"][z]
                        > LoadBusByPhase[p]["VMin-Dem-UP"][z]
                    ):
                        u[z] = (
                            (0.94 - Vmin[:, p])
                ...
                Violations, VmaxFlag, VminFlag, ThermalFlag, n, Locations = sensitivity(
                    LoadsIn,
                    P,
                    Gen,
                    Loadarray,
                ...
                LoadBusByPhase = summary(
                    LoadsIn, Violations, CurMax, Vmax, Vmin, PN, Gen, PO, LB, UB
                )
                print(Vmin[:, p])
            ...
            while Vmax[:, p] > 1.1:
                ...
                for z in UpInd:
                    ########### Turn Down Generation ##########
                    if (
                        LoadBusByPhase[p]["VMax-Dem-UP"][z] < 0
                        and LoadBusByPhase[p]["VMax-Dem-UP"][z]
                        < LoadBusByPhase[p]["VMax-Dem-DOWN"][z]
                    ):
                        u[z] = (
                            -(1.1 - Vmax[:, p])
                            / LoadBusByPhase[p]["VMax-Dem-UP"][z]
                            / (len(UpInd) * grad)
                        )
                ...
                for z in DownInd:
                    ########### Turn Down Demand ##########
                    if (
                        LoadBusByPhase[p]["VMax-Dem-DOWN"][z] < 0
                        and LoadBusByPhase[p]["VMax-Dem-DOWN"][z]
                        < LoadBusByPhase[p]["VMax-Dem-UP"][z]
                    ):
                        u[z] = (
                            -(1.1 - Vmax[:, p])
            ...
            while CurMax[:, p] > 0.5:
                ...
                for z in UpInd:
                    ########### Turn Down Generation ##########
                    if (
                        LoadBusByPhase[p]["Cmax-Dem-UP"][z] < 0
                        and LoadBusByPhase[p]["Cmax-Dem-UP"][z]
                        < LoadBusByPhase[p]["Cmax-Dem-DOWN"][z]
                    ):
                        u[z] = (
                            CurMax[:, p]
                            / LoadBusByPhase[p]["Cmax-Dem-UP"][z]
                            / (len(UpInd) * grad)
                        )
                ...
                for z in DownInd:
                    ########### Turn Down Demand ##########
                    if (
                        LoadBusByPhase[p]["Cmax-Dem-DOWN"][z] < 0
                        and LoadBusByPhase[p]["Cmax-Dem-DOWN"][z]
                        < LoadBusByPhase[p]["Cmax-Dem-UP"][z]
                    ):
                        u[z] = (
                            -CurMax[:, p]
