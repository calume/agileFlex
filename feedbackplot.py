...
# def snapshot_plot(Network_Path,VoltArray,CurArray,RateArray,Trans_kVA,TransRatekVA):
Volts = {}
Currents = {}
...
for p in range(0, 3):
    ...
    print(Trans_kVA / TransRatekVA)
    Currents[p].insert(0, Trans_kVA / TransRatekVA)
    ...
    sm = plt.cm.ScalarMappable(
        cmap=edge_cmap, norm=plt.Normalize(vmin=min(Currents[p]), vmax=max(Currents[p]))
    )
    sm._A = []
    plt.colorbar(
        sm,
        ticks=[
            min(Currents[p]),
            (max(Currents[p]) + min(Currents[p])) / 2,
            max(Currents[p]),
        ],
    )
    plt.show()
    ...
    sm = plt.cm.ScalarMappable(
        cmap=cmap, norm=plt.Normalize(vmin=min(Volts[p]), vmax=max(Volts[p]))
    )
    sm._A = []
    plt.colorbar(
        sm, ticks=[min(Volts[p]), (max(Volts[p]) + min(Volts[p])) / 2, max(Volts[p])]
    )
    plt.show()
    n = n + 1

# snapshot_plot(Network_Path,VoltArray[i],CurArray[i],RateArray,network_summary[i]['Trans_kVA'],TransRatekVA)
