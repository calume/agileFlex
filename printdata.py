...
class printdata(object):
    ...
    def printkeysets(self):
        ...
        f.write('param ChargeEff:= ' + str(self.options['charging_efficiency'])+';\n\n')
        f.write('param DischargeEff:= ' + str(self.options['discharging_efficiency'])+';\n\n')
        ###---set of EVs---
        ...
        if len(self.data["EV"]["name"])!=0:
            ...
            for i in df_flex.index.tolist():
                f.write(str(int(df_flex["EV"][i]))+" "+str(int(df_flex["Window"][i]))+" "+str(df_flex["Start"][i])+"\n")
            f.write(';\n')
            f.write('set EVBoundaryEnd:= \n')
            for i in df_flex.index.tolist():
                f.write(str(int(df_flex["EV"][i]))+" "+str(int(df_flex["Window"][i]))+" "+str(df_flex["End"][i])+"\n")
            f.write(';\n')


            f.write('param SoCStart:= \n')
            for i in df_flex.index.tolist():
                f.write(str(int(df_flex["EV"][i]))+" "+str(int(df_flex["Window"][i]))+" "+str(int(df_flex["Start"][i]))+" "+str(df_flex["SoCStart"][i])+"\n")
            f.write(';\n')
            f.write('param SoCEnd:= \n')
            for i in df_flex.index.tolist():
                f.write(str(int(df_flex["EV"][i]))+" "+str(int(df_flex["Window"][i]))+" "+str(int(df_flex["End"][i]))+" "+str(df_flex["SoCEnd"][i])+"\n")
            f.write(';\n')
        ...
        f.write(';\n')
        f.write('set TRed:= \n')
        for i in self.data["timeseries"]['timeperiod'][1:]:
            f.write(str(i) + "\n")
        f.write(';\n')
        f.close()
    ...
    def printEV(self):
        ...
        f.write('param PGmin:=\n')
        for i in self.data["genmin"]:
           if 'time' not in i:
              for j in self.data["genmin"][i].index.tolist():
                 f.write(str(i)+" "+str(self.data["genmin"]["timeperiod"][j])+" "+str(float(self.data["genmin"][i][j])/self.data["baseMVA"]["baseMVA"][0])+"\n")
        f.write(';\n')
        #----Timeseries of Gen UB-----#
        f.write('param PGmax:=\n')
        for i in self.data["genseries"]:
           if 'time' not in i:
              for j in self.data["genseries"][i].index.tolist():
                 f.write(str(i)+" "+str(self.data["genseries"]["timeperiod"][j])+" "+str(float(self.data["genseries"][i][j])/self.data["baseMVA"]["baseMVA"][0])+"\n")
