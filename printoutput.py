...
#                ind += 1
        ...
        for t in self.instance.T:
            for g in self.instance.Gbs:
                ...
                'PGLB(kW)':self.instance.PGmin[g[1],t]*self.instance.baseMVA,\
                'pG(kW)':round(self.instance.pG[g[1],t].value*self.instance.baseMVA,3),\
                'PGUB(kW)':self.instance.PGmax[g[1],t]*self.instance.baseMVA})
                ind += 1
       #EVs data
        ind = 0
        for (ev,w,t) in self.instance.FlexTimes:
            EV.loc[ind] = pd.Series({'Time period':t,'name':ev,\
                'Window':w,'Charging(kW)':self.instance.pEVCharge[ev,w,t].value,\
                'Discharging(kW)':self.instance.pEVDischarge[ev,w,t].value,'SoC(kWh)':self.instance.SoC[ev,w,t].value})
            ind += 1
