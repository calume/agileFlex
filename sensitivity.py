...
def sensitivity(
    LoadsIn, P, Gen, Loadarray, CurArray, VoltArray, RateArray, CurMax, Vmax, Vmin, n
):
    ...
    for p in range(0, 3):
        ...
        if CurMax[:, p] > 0.5 or Vmin[:, p] < 0.94 or Vmax[:, p] > 1.1:
            ...
            for i in range(0, len(phaseInd)):
                ...
                CurPercPercDown[i] = (CurMaxSensDown[i, p] - CurMax[:, p]) / abs(
                    u[phaseInd[i]]
                )  # abs(u[phaseInd[i]])   #If better this is Negative Number
                VminPercPercDown[i] = (VminSensDown[i, p] - Vmin[:, p]) / abs(
                    u[phaseInd[i]]
                )
                VmaxPercPercDown[i] = (VmaxSensDown[i, p] - Vmax[:, p]) / abs(
                    u[phaseInd[i]]
                )

                ###### If downward demand didnt help, run downward generation for export constraint ########
                if (
                    CurMaxSensDown[i, p] > CurMax[:, p]
                ):  # If CurPercPercDown[i] is positive then max has increased, made worse
                    u = 0 * P
                    ug = 0 * Gen
                    # u[phaseInd[i]] = -abs(perc*P[phaseInd[i]])  #Load is added, P -- u = P + u
                    ug[phaseInd[i]] = abs(
                        perc * Gen[phaseInd[i]]
                    )  # Generation turned down Gen-ug
                    n = n + 1
                    (
                        LS,
                        CS,
                    ...
                    CurPercPercUp[i] = (CurMaxSensUp[i, p] - CurMax[:, p]) / abs(
                        ug[phaseInd[i]]
                    )
                    VminPercPercUp[i] = (VminSensUp[i, p] - Vmin[:, p]) / abs(
                        ug[phaseInd[i]]
                    )
                    VmaxPercPercUp[i] = (VmaxSensUp[i, p] - Vmax[:, p]) / abs(
                        ug[phaseInd[i]]
                    )
