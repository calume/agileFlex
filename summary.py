...
def summary(LoadsIn, Violperphase, CurMax, Vmax, Vmin, PN, Gen, PO, LB, UB):
    ...
    for p in range(0, 3):
        ...
        for i in range(0, len(LoadBus)):
            LoadBus["X"][i] = float(
                Coords["X"][int(LoadBus["Bus"][i]) == Coords["Node"]].values
            )
            LoadBus["Y"][i] = float(
                Coords["Y"][int(LoadBus["Bus"][i]) == Coords["Node"]].values
            )
        ...
        for i in range(0, len(LoadBus)):
            ...
            if Vmax[:, p] > 1.1 and CurMax[:, p] > 0.5:
                if (
                    LoadBus["VMax-Dem-UP"][i] < -tol
                    and LoadBus["Cmax-Dem-UP"][i] < -tol
                ):
                    LoadBus["Action"][i] = "C/VU+"  # +(Vmax/Cmax)'
                if (
                    LoadBus["VMax-Dem-DOWN"][i] < -tol
                    and LoadBus["Cmax-Dem-DOWN"][i] < -tol
                ):
                    LoadBus["Action"][i] = "C/VU-"  # +(Vmax/Cmax)'
            if Vmin[:, p] < 0.94 and CurMax[:, p] > 0.5:
                if (
                    LoadBus["VMin-Dem-DOWN"][i] > tol
                    and LoadBus["Cmax-Dem-DOWN"][i] < -tol
                ):
            ...
            if Vmin[:, p] < 0.94 and Vmax[:, p] > 1.1:
                if (
                    LoadBus["VMax-Dem-DOWN"][i] < -tol
                    and LoadBus["VMin-Dem-DOWN"][i] > tol
                ):
